...
/**
 * This writes to stdout all context keys for answers not covered by the 
 * provided response.
 * It is based on the official TAC scorer (see below).
 */
...
public class SFRecallAnalyse {
  ...
  //  mapping from entity_id:slot_name --> set of equivalence classes for answers contained in response
  //static Multimap<String, Set<Integer>> queryToFoundEClasses = HashMultimap.create();
  static Map<String, Set<Integer>> queryToFoundEClasses = new HashMap<String, Set<Integer>> ();
  ...
  public static void main (String[] args) throws IOException {
    ...
    if (args.length < 2 || args.length > 7) {
      System.err.println ("SlotScorer must be invoked with 2 to 8 arguments:");
      System.err.println ("\t<responses file>  <key file> [flag ...]");
      System.err.println ("flags:");
      System.err.println ("\ttrace  -- print a line with assessment of each system response");
      System.err.println ("\tanydoc -- judge response based only on answer string, ignoring doc id and justification offsets");
      System.err.println ("\tignoreoffsets -- judge response based on answer string and docid, ignoring justification offsets");
      System.err.println ("\tnocase -- ignore case in matching answer string");
      System.err.println ("\tslots=<slotfile> -- take list of entityId:slot pairs from slotfile");
      System.err.println ("\t                    (otherwise list of pairs is taken from system responses)");
      System.exit(1);
    }
    ...
    try {
      ...
      System.err.println ("Unable to open judgement file " + keyFile);
      System.exit (1);
    }
    String line;
    while ((line = keyReader.readLine()) != null) {
      ...
      if (fields.length != 12) {
        System.err.println ("Warning: Invalid line in judgement file:");
      ...
      try {
        ...
        System.err.println ("Warning: Invalid line in judgement file -- invalid equivalence class:");
        System.err.println (line);
        continue;
      }
      if (eclass == 0)
      ...
      if (J != null) {  // this may happen under lenient matching: nocase, ignoreoffsets, or anydoc
        ...
        if (! jment.equals(J)) {  
          ...
          System.err.println("Warning: Multiple conflicting judgments for response " + key + ": " + jment + " vs. " + J + " => we kept " + strongerJ);
    ...
    System.err.println ("Read " + judgement.size() + " judgements.");
    ...
    try {
      ...
      System.err.println ("Unable to open responses file " + responseFile);
      System.exit (1);
    }
    // String line;
    while ((line = responseReader.readLine()) != null) {
      ...
      if (fields.length < 4 | fields.length > 10) {
        System.err.println ("Warning: Invalid line in responses file:  " + fields.length + " fields");
        System.err.println (line);
        continue;
      }
    ...
    System.err.println ("Read responses for " + response.size() + " slots.");
    ...
    for (String query : slots) {
      ...
      if (query_eclasses.get(query) != null) {
        if (type == "list")
          ...
          System.err.println ("Warning: unrecognizable slot type " + query);
          continue;
        }
      }
      ...
      if (responseList == null) {
        System.err.println ("Warning: No system response for slot " + query);
        continue;
      }
      ...
      if (type == "single") {
        if (num_responses_to_query > 1) {
          System.err.println ("Warning: Ignoring all but first response among multiple responses for single-valued slot " + query);
          responseList = responseList.subList(0,0);
          num_responses_to_query = responseList.size();
          if (num_responses_to_query != 1) {
            System.err.println ("Error: unable to take first of multiple responses for single-valued slot for query " + query);
            System.exit (1);
          }
        }
      }
      ...
      for (String responseString : responseList) {
        ...
        if (doc_id.equals("NIL")) {
          if (num_responses_to_query > 1)
            System.err.println ("Warning: More than one response, including NIL, for " + query);
          ...
          String J = judgement.get(key);
          if (J == null) {
            System.err.println ("Warning: No judgement for " + key);
            J = WRONG;
          }
          symbol = J;
          if (J.equals(IGNORE) || J.equals(WRONG)) {
            ...
            System.err.println ("ERROR: Invalid judgement " + J);
            System.exit (1);
          }
        }
        if (trace)
          System.err.println (symbol + " " + query + " " + responseString);
      }
      // Collect distincts for later recall analysis.
      queryToFoundEClasses.put(query, distincts);      
    }

    int numNotCorrectInKey = 0;
    int numCorrectCoveredContexts = 0;
    int numCorrectUncoveredContexts = 0;
    
    for (String key : judgement.keySet()) {
      if (equivalenceClass.containsKey(key)) {
        int ec = equivalenceClass.get(key);
        String[] fields = key.split(":",4);
        String query_id = fields[0] + ":" + fields[1] + ":" + fields[2];
        if ((query_eclasses.containsKey(query_id) && 
              query_eclasses.get(query_id).contains(ec)) || 
              (query_kb_eclasses.containsKey(query_id) && 
                  query_kb_eclasses.get(query_id).contains(ec))) {
          
          if (queryToFoundEClasses.get(query_id).contains(ec)) {
            numCorrectCoveredContexts++;
          } else {
            System.out.println(key);
            numCorrectUncoveredContexts++;
          }
        } else {
          numNotCorrectInKey++; 
        }
    ...
    System.err.println (" Contexts not jugded 'correct' in key: " + numNotCorrectInKey);
    System.err.println (" Contexts jugded 'correct' in key and covered by response: " + numCorrectCoveredContexts);
    System.err.println (" Contexts jugded 'correct' in key and not covered by response: " + numCorrectUncoveredContexts);
    
    System.err.println ();
    if (slotFile != null)
      System.err.println ("Slot lists taken from file " + slotFile);
    else
      System.err.println ("Slot lists taken from system responses");
    System.err.println ("Slot lists include " + svSlots.size() + " single valued slots");
    System.err.println ("               and " +  lSlots.size() + " list-valued slots");
    System.err.println ("\nNumber of filled slots in key that are not in reference KB: " + num_answers);
    System.err.println ("Number of filled slots in key that are in reference KB: " + num_kb_answers);
    System.err.println ("\nNumber of filled slots in responses: " + num_responses);
    System.err.println ("\tNumber Correct (not in reference KB): " + num_correct);
    System.err.println ("\tNumber Redundant with reference KB: " + num_kb_redundant);
    System.err.println ("\tNumber redundant with another response: " + num_redundant);
    System.err.println ("\tNumber inexact: " + num_inexact);
    System.err.println ("\tNumber incorrect / spurious: " + num_wrong);
    ...
    System.err.println ("\tDiagnostic Recall: " + num_correct + " / " + num_answers + " = " + recall);
    System.err.println ("\tDiagnostic Precision: " + num_correct + " / (" + num_responses + "-" + num_kb_redundant + ") = " + precision);
    System.err.println ("\tDiagnostic F1: " + F);
    ...
    System.err.println ("\tRecall: (" + num_correct + "+" + num_kb_redundant + ") / (" + num_answers + "+" + num_kb_answers + ") = " + recall);
    System.err.println ("\tPrecision: (" + num_correct + "+" + num_kb_redundant + ") / " + num_responses + " = " + precision);
  ...
  static List<String> readLines (String fileName) {
    ...
    try {
      ...
      System.err.println ("Error readng from file " + fileName);
      System.exit (1);
    }
    System.err.println ("Read " + lines.size() + " lines from " + fileName);
    return lines;
  }
  ...
  private static String qidFromKey(String key) {
    ...
    //System.err.println("Using qid " + qid + " from key " + key);
    return qid;
  }
